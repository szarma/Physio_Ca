...
javabridge.start_vm(class_path=javabridge.JARS)
import numpy as np
...
def importFirstFrames(rdr,idx,n=100,channel=None):
    ...
    for i in nn:
        try:
            ...
            warn("For %i series, you asked for %i first frames, but there was only %i so I give you that."%(idx,n,len(image)))
            break
    image = np.stack(image)
    return image
...
if __name__=="main":
    ...
    if iSeries is not "all":
        ...
        if iSeries>=Nimages:
            raise ValueError("""
                             iSeries (%i) needs to be smaller than the
                             total number of total series %i"""%(iSeries, Nimages))
