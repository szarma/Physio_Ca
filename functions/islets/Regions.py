...
#             self.reassign_spikes(slightly_blurred_image+self.statImages[self.mode])
#         except:
        self.df["peakValue"] = [image[p] for p in B_]
        self.ExcludePixels = excludePixels
        self.update()
    
    def reassign_spikes(self, image, write=True):
        newPeaks = []
        newValues = []
...
#                         y[nans]= np.interp( x(nans), x(~nans), absSlow[j][~nans] )
    ...
    def calc_spikes(self, ts, z_th=3, Npoints=None, smooth=None, verbose=False, save=True, t=None, zScores=None):
        from .numeric import runningAverage
        from scipy.signal import find_spikes, peak_widths
        if zScores is None:
            if "zScore_%g"%ts not in self.df.columns:
                if verbose:
                    print ("filtering...")
