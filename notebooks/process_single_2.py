...
parser.add_argument('--only-movie', const=True, default=False, action="store_const",
                    help='only do movie')
parser.add_argument('--line-scan', const=True, default=False, action="store_const",
                    help='indicate if it is a line scan')
...
if args.line_scan:
    ...
    for ix, name in zip(indices,serNames):
        ...
        rec.import_series(name, isLineScan=rec.metadata.loc[ix,"SizeY"]==1)
        linescan = LineScan(
            data = rec.Series[name]["data"].astype("float32").sum(1).T,
