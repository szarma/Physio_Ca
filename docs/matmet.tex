...
\begin{figure}[t]
    ...
    \begin{minipage}{.49\textwidth}
        {\fontfamily{phv}\selectfont A} \includegraphics[scale=.5,trim=13mm 0 0 0,clip,align=t]{figures/z_score_1.pdf}
    \end{minipage}
    \begin{minipage}{.49\textwidth} 
        {\fontfamily{phv}\selectfont B} \includegraphics[scale=.5,trim=15mm 30mm 0 0,clip,align=t]{figures/z_score_2.pdf}
        {\fontfamily{phv}\selectfont C} \includegraphics[scale=.48,trim=5mm 0mm 0 0,clip,align=t]{figures/z_score_3.pdf}
    \end{minipage}
    \caption{
...
%  \newgeometry{textwidth=20cm}
 ...
 \caption{(A) Example of a track (black) and slow components in sequential filtering (shades of gray). Square indicates the regions shown in B and C. 
 (B) Close-up view of a small region (top) and a representation of all candidate events in that region detected through sequential filtering.
 Height corresponds to the timescale at which the event was detected.
...
We devised an algorithm to detect events at timescales spanning multiple orders of magnitude.
...
We also neglect events that last less than $3$ time frames, as well as those too close to the begging or end of the recording (within $\delta t/2$), which we ascribe to artefacts from zero-padding for the filtering. 
