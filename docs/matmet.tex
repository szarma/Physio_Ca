...
%  \newgeometry{textwidth=20cm}
 ...
 In red we show events that are deemed false positives, because they are detected at too few filtering timescales.
 Others events belong to groups of similar events which are then considered real; in black we show the event closest to the one finally distilled. 
 It's time boundaries are set to median of the whole group.
 ...
 We choose a non-linear color code (mapping to a hill curve) yet nearly linear between $1$ and $\sim 5$ seconds, to emphasize the color difference at the timescales most interesting for our analysis.
 }
